...
当给定一个范围时，`:!{cmd}` 命令就有了不同的含义。由 `[range]` 所指定的行会传给 `{cmd}` 作为标准输入，并把 `{cmd}` 的输出结果覆盖 `[range]` 内原本的内容。即，`[range]` 内的文本会被指定的 `{cmd}` 进行过滤。（`:h :range!`）

比如，要用 sort 对 csv 的第二个字段排序，可以使用：

```
:2,$!sort -t',' -k2
```

Vim 提供了一种方便的快捷方式来设置 `:[range]!{filter}` 中的范围。我们可以用 `!{motion}` 命令符切换到命令行模式，并把 `{motion}` 所涵盖的范围预置在命令行上。（`:h !`）

## 第6章 管理多个文件

参数列表是缓冲区列表的强力补充。使用 `:args` 命令可以把缓冲区列表中的文件分组。可以用 `:argdo` 命令在列表中的每个文件上执行 Ex 命令。
...
`:args {arglist}` 填充参数列表，可以使用文件名，也可以使用通配符。参见（`:h args_f`）
...
参数列表是 vi 的一个功能，而缓冲区列表是 vim 引入的增强功能。因此，参数列表看上去比较简陋。

用反引号结构指定文件。`:h backtick-expansion`
